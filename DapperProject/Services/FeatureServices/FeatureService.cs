...
namespace DapperProject.Services.FeatureServices
{
    public class FeatureService : IFeatureService
    {
        ...
        {
            var query = "insert into Feature (Title,Description,ImageUrl,IframeUrl) values (@Title,@Description,@ImageUrl,@IframeUrl)";
        ...
        {
            var query = "delete from Feature where FeatureId = @id";
        ...
        {
            var query = "select * from Feature where FeatureId = @id";
            var parametres = new DynamicParameters();
            parametres.Add("@id", id);
            var connection = _context.CreateConnection();
        ...
        {
            var query = "update Feature set Title = @Title,Description = @Description,ImageUrl = @ImageUrl,IframeUrl = @IframeUrl where FeatureId = @FeatureId";
            var parametres = new DynamicParameters(FeatureDto);
            var connection = _context.CreateConnection();
            await connection.ExecuteAsync(query, parametres);
        }
